...
export function buildDemoProps(template, solution) {
  return async function getServerSideProps({ query }) {
    ...
    const images = await Promise.all(config.images.map(async (img) => {
      ...
      // Cache-buster suffix for re-measure: appended to Scene7 URLs to bypass /_next/image cache
      const cb = cacheBuster ? `&_t=${cacheBuster}` : '';
      const fullUrl = buildImageUrl(img.uuid, { width: fullWidth, quality: 80 }) + cb;
      const blurUrl = buildBlurUrl(img.uuid, { width: fullWidth, quality: 5 }) + cb;
